defmodule GraphqlPracticeWeb.Schema.Mutations.UserTest do
  ...
  describe "@create_user" do
    test "Creates a new user" do
      ...
      assert {:ok, %{data: data}} =
        Absinthe.run(@create_user_doc, Schema, variables: %{"name" => create_name, "email" => create_email})
      ...
      assert %User{name: ^create_name, email: ^create_email, id: ^id} = Accounts.get_user!(user_id)
    end
    ...
    test "Returns errors when invalid input arguments are provided" do
      ...
      assert {:ok, %{errors: errors}} =
        ...
        assert [%{details: details, message: message, path: path}] = errors
    end
  end
end
